...
import gensim
from deep_learn import constants, attention_layer, fetch_data
...
async def clean_str(strings : str) -> str:
    """
...
def clean_reviews(reviews : str) -> pd.Series:
    event_loop = asyncio.new_event_loop()
    asyncio.set_event_loop(event_loop)
    tasks = (clean_str(review) for review in reviews)
    try:
...
def tokenize_sentences(reviews : pd.Series) -> None:
