...
async def clean_data(text : str, remove_stops : bool = False, stemming : bool = False, lemmatization : bool = False) -> str:
...
def clean_texts(texts : pd.Series) -> pd.Series:
    ...
    tasks = ()
    tasks = (clean_data(text, True, False, True) for text in texts)
    try:
...
def get_ner_tagged_text(text : str) -> str:
    tokens = spacyNlp(text)
...
def ner_tagging_replacement(texts) -> list:
    tagged_txts= [get_ner_tagged_text(text) for text in texts]
    return tagged_txts


def train_categorizer(train_data : pd.DataFrame) -> None:
    train_data.text = clean_texts(train_data.text)
    train_data.text = ner_tagging_replacement(train_data.text)
